...
export async function generatePropertyQueryPrompt(
  ...
  if (locationKeywords.cities.length > 0 || locationKeywords.districts.length > 0) {
    ...
    promptParts.push('')
    promptParts.push('The tool uses intelligent multi-strategy location matching:')
    promptParts.push('- **Strategy 1 - Fuzzy Address Matching**: Searches full addresses (street + area + city + postcode)')
    promptParts.push('  - Exact, substring, fuzzy (60% threshold - handles typos), and phonetic matching')
    promptParts.push('  - Example: "Baeker Street" → "Baker Street" (phonetic), "Sherrif" → "Sheriff" (fuzzy)')
    promptParts.push('- **Strategy 2 - Geocoded Area Boundaries**: Uses Google Places API to understand area extents')
    promptParts.push('  - Finds properties within actual geographic boundaries of areas')
    promptParts.push('  - Example: "central Edinburgh" → all properties within central Edinburgh bounds')
    promptParts.push('  - Example: "Spinningfields" → properties within Spinningfields area boundaries')
    promptParts.push('- **Strategy 3 - Hybrid**: Runs both strategies in parallel and merges results by relevance')
    promptParts.push('  - Properties matching both strategies are ranked highest')
    promptParts.push('  - Returns best matches sorted by combined fuzzy + proximity scores')
    promptParts.push('- **Fallback**: If no match found, returns top 10-15 similar locations as refinements')
    promptParts.push('- Example: User says "Londn" → system matches to "London" automatically (fuzzy)')
    promptParts.push('- Example: User says "Spinningfields" → finds properties via both address matching AND geographic bounds (hybrid)')
  ...
  promptParts.push('')
  promptParts.push('**Customer**: "Do you have any properties on Baker Street?"')
  promptParts.push('')
  promptParts.push('**Search with**:')
  promptParts.push('- transaction_type: "rent" (or "sale", depending on earlier conversation)')
  promptParts.push('- location: "Baker Street"')
  promptParts.push('')
  promptParts.push('**Customer**: "I want to live in Spinningfields"')
  promptParts.push('')
  promptParts.push('**Search with**:')
  promptParts.push('- transaction_type: "rent" (or "sale", depending on earlier conversation)')
  ...
  promptParts.push('1. Exact match: "Baker Street" → "Baker Street, Manchester, M1" ✓')
  promptParts.push('2. Substring: "Baker" or "Manchester M1" → "Baker Street, Manchester, M1" ✓')
  promptParts.push('3. Fuzzy: "Bker Street" → "Baker Street, Manchester, M1" ✓ (60%+ similar - typo tolerant)')
  promptParts.push('4. Phonetic: "Baeker Street" → "Baker Street, Manchester, M1" ✓ (sounds similar)')
  promptParts.push('')
  promptParts.push('**Geocoded Boundary Strategy**: Uses Google Places API to find properties within area bounds')
  promptParts.push('1. Area search: "Spinningfields" → finds ALL properties within Spinningfields geographic boundary')
  promptParts.push('2. District search: "central Edinburgh" → finds properties within central Edinburgh bounds (not just circle)')
  promptParts.push('3. Landmark search: "Kensington" → uses actual Kensington area boundaries')
  promptParts.push('')
  promptParts.push('**Hybrid**: Both strategies run in parallel, results merged and ranked by relevance')
  promptParts.push('- Properties matching BOTH strategies ranked highest (boosted relevance)')
  promptParts.push('- Otherwise ranked by combined fuzzy similarity + geographic proximity score')
  promptParts.push('')
  promptParts.push('You can search by: street names, area names, districts, landmarks, "central [city]", or any part of address.')
  promptParts.push('If no match found, tool returns top 10-15 most similar locations by fuzzy/phonetic + geocode score.')
  promptParts.push('')
